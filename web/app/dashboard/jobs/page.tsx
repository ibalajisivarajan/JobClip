    ...
    <section>
      <div className="mb-6 flex items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-600">Jobs captured from your Chrome extension.</p>
        </div>
      </div>
        ...
        <table className="min-w-full divide-y divide-slate-200 text-sm">
          <thead className="bg-slate-100 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">
            <tr>
              ...
              <th className="px-4 py-3">Remote/Hybrid</th>
            ...
            {(jobs ?? []).map((job: Job) => (
              <tr key={job.id} className="hover:bg-slate-50">
                ...
                <td className="px-4 py-4">
                  <Link className="font-semibold text-blue-700 hover:underline" href={`/dashboard/jobs/${job.id}`}>
